...
numDistortion_function <- function (source, dest_list) {
  ...
  dest_list <- list_append(dest_list,  
                           memorySize_function (source, iter_g, dist_num_g))
}
...
repl_matrix_A <- matrix(sample(c(0,1), iter_g * iter_g, replace =TRUE, prob=c(.3, .7)), ncol = iter_g)
for (k in 1:iter_g)
{
...
repl_matrix_B <- matrix(sample(c(0,1), iter_g * iter_g, replace =TRUE, prob=c(.3, .7)), ncol = iter_g)
...
comb_matrix <- matrix(ncol=iter_g, nrow=iter_g)

for (k in 1:dist_num_g)
{
  for(j in 1:iter_g){
...
for (k in (dist_num_g + 1) : iter_g)
{
  for(j in 1:iter_g){
      comb_matrix[k, j] <- learned_matrix_B[k - dist_num_g, j]
    }
}

desired_prob_A <-  learned_matrix_A[dist_num_g + 1,]
desired_echo_A <- get_echo(desired_prob_A, comb_matrix) 

#MINERVA
cos_cor_minerva <- cosine(desired_echo_A, desired_prob_A)

#Prototype Classifier
cos_cor_prototype <- cosine(learned_matrix_A[dist_num_g + 1,], A)
...
for (k in (dist_num_g + 1) :iter_g)
{
  ...
  cos_list_minerva <- list_append(cos_list_minerva, cosine(desired_echo_A, desired_prob_A))
...
#MINERVA Old
cos_cor_minerva_Old <- cosine(desired_echo_A_Old, desired_prob_A_Old)
...
for (k in 1:dist_num_g)
{
  ...
  cos_list_minerva_Old <- list_append(cos_list_minerva_Old, cosine(desired_echo_A_Old, desired_prob_A_Old))
...
#MINERVA Old
cos_cor_minerva_Old_B <- cosine(desired_echo_B_Old, desired_prob_B_Old)
...
for (k in 1:dist_num_g)
{
  ...
  cos_list_minerva_b_Old <- list_append(cos_list_minerva_b_Old, cosine(desired_echo_B_Old, desired_prob_B_Old))
...
category <- rep(c("A","B"),each=dist_num_g)

old_new <- rep(c("OLD","NEW"),each=dist_num_g)

sim_A <- as.numeric(as.character(unlist(cos_list_minerva_comb)))
sim_B <- as.numeric(as.character(unlist(cos_list_minerva_comb_B)))

comb_simA_simB <- append(sim_A, sim_B)

label_x <- rep(c("Old A", "New A", "Old B", "New B"), each = 50)

plot_df <- data.frame(label_x, comb_simA_simB)
...
category_prot <- rep(c("A","B"),each=dist_num_g)
old_new_prot <- rep(c("OLD","NEW"),each=dist_num_g)
